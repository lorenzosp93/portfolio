<template>
  ...
  <ol v-if="data.length > 0" class="relative grid grid-flow-row grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mx-10 my-8">
    <skills-category class="hover:scale-102.5 transition duration-300 ease-in-out " v-for="category in data" :key="category.name" v-bind="category" :isActive="isActive"> </skills-category>
  </ol>
    </template>
  </resume-panel>
</template>
