<template>
  <div class="container min-h-[80vh] lg:min-h-screen relative flex w-screen mx-auto flex-wrap">
    ...
    <div class="p-auto gap-6 container relative w-screen flex overflow-x-scroll no-scrollbar snap-x snap-mandatory m-auto scroll-smooth scroll-px-1" id="blog-container">
      ...
      <retry-button v-else-if="!data.length" @load-entries="loadEntries" />
    </div>
  </div>
</template>
...
export default {
  components: {ListCard, RetryButton},
  name: 'TheBlog',
  data () {
    return {
      isLoading: false,
