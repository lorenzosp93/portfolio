<template>
  <div @click="toggleDetails" class="snap-center m-5 container flex-none rounded-2xl border-1 border-gray-200 shadow-md dark:bg-gray-600 bg-white" :class="{'pointe-events-none': detailsVisible}">
    ...
    <blog-entry-detail v-if="type == 'blog' && isActive" :isOpen="detailsVisible" @card-closed="toggleDetails"  :name="name" :created_at="created_at" :created_by="created_by" :location="location" :picture="picture" :content="content" :attachments="attachments"/>
    
    <project-entry-detail v-if="type == 'project' && isActive" :isOpen="detailsVisible" @card-closed="toggleDetails"  :name="name" :location="location" :picture="picture" :content="content" :status="status" :attachments="attachments" />
